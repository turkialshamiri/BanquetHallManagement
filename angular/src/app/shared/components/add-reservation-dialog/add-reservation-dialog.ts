...
import { calculateReservationPrice } from 'src/app/core/utils/reservation-price.util';
...
/** Shared MatDialog options for responsive, scrollable reservation dialogs. */
export const ADD_RESERVATION_DIALOG_CONFIG = {
  width: 'min(760px, 95vw)',
  maxWidth: '95vw',
  maxHeight: '90dvh',
  panelClass: 'add-reservation-dialog-panel',
  disableClose: true,
  autoFocus: 'first-tunable-element',
} as const;
...
export class AddReservationDialog implements OnInit {
  ...
  get selectedHall(): Hall | undefined {
    return this.halls.find((hall) => hall.id === this.form.hallId);
  }
  ...
  onFormFieldChange(field?: keyof ReservationFormErrors): void {
    if (field) {
  ...
  private loadDropdownData(): void {
    this.canSelectServices = this.policy.hasSnapshot(
      'BanquetHallManagement.Services'
    );

    const emptyServices: PagedServiceResult = { items: [], totalCount: 0 };

    forkJoin({
      customers: this.customerService.getCustomers(),
      halls: this.hallService.getHalls(),
      services: this.canSelectServices
